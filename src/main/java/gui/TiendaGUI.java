...
public class TiendaGUI extends JFrame {
    ...
    private void mostrarInfoMascota(Mascota mascota) {
        ...
        btnAlimentar.addActionListener(e -> {
            if (alimentarMascotaSeleccionada(mascota)) {
                refrescarMascotas();
    ...
    private JComponent construirTabHabitats() {
        ...
        btnComprar.addActionListener(e -> {
            ...
            try {
                ...
                tienda.agregarHabitatAlCarrito(carrito, tipo);
    ...
    private JComponent tarjetaHabitat(Habitat h) {
        ...
        int capacidadMaxima = h.getTipo().getCapacidadMaxima();
        JProgressBar barra = new JProgressBar(0, capacidadMaxima);
        barra.setValue(h.getOcupados());
        barra.setStringPainted(true);
        barra.setString("👤 " + h.getOcupados() + " / " + capacidadMaxima + " ocupados");
    ...
    private boolean alimentarMascotaSeleccionada(Mascota mascota) {
        Alimento alimentoCompatible = null;
        for (Alimento alimento : usuario.getAlimentos()) {
            if (alimento.esCompatibleCon(mascota.getTipoMascota())) {
                alimentoCompatible = alimento;
                break;
            }
        }

        if (alimentoCompatible == null) {
            mostrarError("No tienes alimento compatible para " + mascota.getNombre());
            return false;
        }

        mascota.alimentar(alimentoCompatible);
        usuario.getAlimentos().remove(alimentoCompatible);
        return true;
    }

    private void limpiarHabitatDeMascota(Mascota mascota) {
        Habitat habitat = mascota.getHabitatAsignado();
        if (habitat == null) {
            mostrarError(mascota.getNombre() + " no tiene habitat asignado");
            return;
        }
        habitat.limpiar();
    }
